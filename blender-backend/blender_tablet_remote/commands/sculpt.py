...
from ..sculpt_samples import BrushSamples
from . import command
...
def _location(obj, point, rv3d):
    ...
    if visible:
        return inv @ location if target.original == obj else None
...
def _preview():
    ...
    _undo_preview()
    obj = _mesh()
    points = _stroke['points'] + ([_stroke['tail']] if _stroke.get('tail') else [])
    if not points:
        return
    ...
    try:
        ...
        if records:
            with bpy.context.temp_override(window=window, area=area, region=region):
                # Defer the native undo commit until the object matrix is restored.
                # FINISHED also occurs when native sculpt rejects every sample
                # (for example on geometry generated by Mirror), without creating
                # a sculpt step. Replaying must never undo an unrelated earlier step.
                result = bpy.ops.sculpt.brush_stroke('EXEC_DEFAULT', False, stroke=records,
        ...
        if obj.matrix_world != original:
            ...
            bpy.context.view_layer.update()
    if records:
        # Commit the pending native Sculpt step, or a reversible unchanged state
        # when no native stroke started. Exactly one owned step per preview.
        with view3d_override():
            bpy.ops.ed.undo_push(message='Trazo de Escultura')
...
def _refresh_surface():
    obj = bpy.context.view_layer.objects.active
    if obj and obj.type == 'MESH' and obj.mode == 'SCULPT':
        # Offscreen SOLID can retain an old sculpt surface although Mesh vertices
        # already changed. Grid/BMesh sculpt keep separate native backing data;
        # updating their base mesh here would overwrite the live preview.
        if not obj.use_dynamic_topology_sculpting and not any(m.type == 'MULTIRES' for m in obj.modifiers):
            obj.data.update()
        found = find_view3d()
        if found: found[1].tag_redraw()
...
def stroke(payload):
    ...
    if phase == 'begin':
        ...
        cancel_all()
        brush_id = 'SMOOTH' if payload.get('smooth') else _settings['brush']
        brush = _activate(brush_id)
        _stroke = dict(id=identity, owner=owner, points=[], preview=False,
                       brush=brush_id,
                       invert=bool(payload.get('invert')), multires_baseline=None,
                       symmetry=status()['symmetry'], detail=status()['dyntopo']['detail'], tail=None, tail_input=None)
        found = find_view3d()
        aspect = found[2].width / max(found[2].height, 1) if found else 1.0
        _stroke['sampler'] = None if _stroke['brush'] == 'GRAB' else BrushSamples(
            _settings['radius'] * 2 * brush.spacing / 100, aspect, _settings['pressure_size'])
        obj = _mesh()
    ...
    tail = None
    if _stroke['sampler'] is not None:
        try:
            points, tail = _stroke['sampler'].feed(points, 4096-len(_stroke['points']))
    ...
    if mapped:
        ...
        _stroke['tail'] = None
        _stroke['tail_input'] = None
    tail_key = (tail['u'], tail['v'], tail['pressure']) if tail else None
    if tail and tail_key != _stroke['tail_input']:
        tail_hits = _map_samples([tail])
        if tail_hits:
            _stroke['tail'] = tail_hits[-1]
            _stroke['tail_input'] = tail_key
            changed = True
    if changed:
        try:
            _preview()
            ...
            cancel()
            raise
    return _result(active=True, stroke_id=identity, hit=changed)
